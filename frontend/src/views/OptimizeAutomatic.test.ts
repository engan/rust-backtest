...
beforeEach(() => {
  ...
  mocks.fetchBinanceKlines.mockResolvedValue(Array.from({ length: 400 }, () => ({})))
  ...
  mocks.createResearchJob.mockImplementation(async (request) => ({
    id: `research-${request.parameterGrid.base.params.sl_tp_method}`,
    ...
    return {
      ...
      report: {
        ...
        walk_forward: {
          ...
          windows: [0, 1, 2, 3].map((window_index) => ({ window_index, selected_candidate_index: 0, validation_summary: { pnl_total: result.pnl / 4, total_trades: result.trades / 4 }, validation_closed_trade_pnls: [result.pnl / 4] })),
        },
      },
    }
  })
...
describe('one-click automatic optimization', () => {
  it('runs all four methods on shared data and selects the eligible OOS leader', async () => {
    ...
    const requests = mocks.createResearchJob.mock.calls.map(([request]) => request)
  ...
  it('explains a too-short validation window before submitting any jobs', async () => {
    ...
    expect(wrapper.text()).toContain('must exceed the largest candidate warmup')
